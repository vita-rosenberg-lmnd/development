...
WITH temp_premium_report_us AS(
    select * 
    FROM pet_finance.premium_report_us
),
...
pet_not_cancelled_monthly_written_premium AS(
         SELECT   SUM(monthly_written_premium) AS sum_of_type,
                  public_id,
                  'pet_not_cancelled_monthly_written_premium' AS errType
         FROM     temp_premium_report_us
         WHERE    public_id IN(
                         SELECT public_id
                         FROM   pet_not_cancelled)
         GROUP BY public_id
         HAVING   (-0.01) < SUM(monthly_written_premium)
                  AND SUM(monthly_written_premium) < (0.01)
), 

pet_not_cancelled_monthly_earned_premium AS(
         SELECT   SUM(monthly_earned_premium) AS sum_of_type,
                  public_id,
         ...
         WHERE    public_id IN(
                         ...
                         FROM   pet_not_cancelled)
         GROUP BY public_id
         HAVING   (-0.01) < SUM(monthly_earned_premium)
...
pet_flat_cancelled_monthly_written_premium AS(
         SELECT   SUM(monthly_written_premium),
                  public_id,
                  'pet_flat_cancelled_monthly_written_premium' errType
         FROM     temp_premium_report_us
         WHERE    public_id IN(
                         SELECT policyId
                         FROM   pet_flat_cancelled)
         GROUP BY public_id
...
monthly_unearned_premium AS(
         SELECT   SUM(monthly_unearned_premium),
                  public_id,
...
pet_inactive_policies_monthly_difference AS(
         SELECT   (SUM(monthly_written_premium) - SUM(monthly_earned_premium)) AS monthly_sum,
                  public_id,
                  'pet_inactive_policies_monthly_difference' AS errType
         FROM     temp_premium_report_us
         WHERE    public_id IN(
                         SELECT public_id
                         FROM   pet_inactive_policies)
         GROUP BY public_id
...
SELECT public_id,
       errType
FROM   pet_not_cancelled_monthly_written_premium
UNION
SELECT public_id,
       errType
FROM   pet_flat_cancelled_monthly_written_premium
UNION
SELECT public_id,
       errType
FROM   pet_flat_cancelled_monthly_earned_premium
UNION
SELECT public_id,
       errType
...
SELECT public_id,
       errType
FROM   pet_active_policies_monthly_written_premium
UNION
SELECT public_id,
       errType
FROM   pet_active_policies_monthly_earned_premium
UNION
SELECT   public_id,
         errType
FROM     pet_inactive_policies_monthly_difference
ORDER BY errType
