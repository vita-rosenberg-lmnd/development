...
--  FROM lemonade.backups.CAR_FINANCE_LOSS_REPORT_RAW_2022_08_30 
  WHERE
    accounting_date < CURRENT_DATE+1
  GROUP BY 1, 2, 3
)

--select *
--from metrics_based_on_source_tables
--where claim_internal_id = 480
--and section = 'salvage'
--;
--
--select *
...
;
;--27,142.97 report
;--38,495.97 source 11,353
---480 unique_transaction_id 583
--LOSSES_INCURRED_DIRECT -3,150
--
--SALVAGE_RECEIVED_DIRECT 3,150
--
--LOSSES_PAID_DIRECT_NET_OF_SALVAGE_SUBRO -3,150
--

create or replace 
table lemonade.backups.CAR_FINANCE_LOSS_REPORT_RAW_2022_08_30
as 
select * from car_finance.loss_report_raw
;
select count(*) from car_finance.loss_report_raw--7,554
;
select count(*) from lemonade.backups.CAR_FINANCE_LOSS_REPORT_RAW_2022_08_30;--7,554
;
--
;
select *
...
--from lemonade.backups.CAR_FINANCE_LOSS_REPORT_RAW_2022_08_30
where 
    claim_internal_id = 480 
    and section = 'salvage'
    --and losses_incurred_direct = -3150
    order by created_at desc
    ;
    ;
    ;
delete from car_finance.loss_report_raw 
where created_at = '2022-08-30 11:11:59.773'
    ;
    ;
...
with duplicates AS (
    ...
    where 
        claim_internal_id = 480
       and (losses_incurred_direct = -11353 AND record_type = 'update')      --OK first run      
       --and losses_incurred_direct = -3150 -- OK second run
       and section = 'salvage'
)
...
SELECT 
  ...
  object_construct('reason', 'fix duplicated record originaly created on \'2022-08-30\', using additional negation recrods', 'type', 'bug_fix') AS comments
...
--UPDATE
  ...
  INSERT INTO  car_finance.loss_report_raw
  --lemonade.backups.CAR_FINANCE_LOSS_REPORT_RAW_2022_08_30
  (
     status, line_of_business, reporting_code, feature_type, general_claim, claim_internal_id,
     catastrophe_number, state_claim_occurred, country_claim_occurred, claim_payor, time_of_loss,
     report_time, transaction_type, accounting_date, cohort_id, cohort, policy, policy_internal_id,
