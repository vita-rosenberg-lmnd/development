...
premium_activities AS (
    SELECT POLICY_ID, USER_ID, ACTIVITY, VALUE, ACTIVITY_DATE, EFFECTIVE_DATE, ACCOUNTING_DATE, METADATA, IDEMPOTENCY_KEY, CREATED_AT
...
policy_ids_with_update_event AS(
select policy_id, effective_date AS event_date, 'update' AS activity
from public.policy_premium_activities a 
where activity='policy_update'
order by activity_date
    ),
policy_ids_with_cancel_event AS(
select policy_id, effective_date AS event_date, 'cancel' AS activity
from public.policy_premium_activities a 
where activity='policy_cancelation'
order by activity_date
),
policy_ids_with_new_business_event AS(
select policy_id, effective_date AS event_date, 'new business' AS activity
from public.policy_premium_activities a 
...
policy_ids_with_reinstate_event AS(
...
select a.policy_id, encrypted_policy_ID, policy_version_id, endorsement_type, 
...
b.activity
from cte_first_version a join policy_ids_with_reinstate_event b on a.policy_id=b.policy_id
